...
class Agent(Node):
    ...
    def __init__(self):
        ...
        # Get the formation shape and reference type from terminal
        formation_shape = os.environ['formation_shape']
        self.reference = os.environ['reference']
        ...
        #set the gain por the proportional action
        self.gain_p = 5
    ...
    def timer_callback(self):
        ...
        if self.all_received:
            ...
            if self.reference != 'point':
                self.pp_des[0] = 3+0.5*np.cos(0.001*self.kk)
                self.pp_des[1] = 3+0.5*np.sin(0.001*self.kk)
                self.pp_des[2] = 5.0
            ...
            self.get_logger().info(f"THE AGENT {self.agent_id} UPDATED ITS STATUS AS: {p_ii_kk}")
